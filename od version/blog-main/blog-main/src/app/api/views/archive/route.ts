...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get current view count for this specific archive post
    ...
    console.error("Error updating archive views:", error);
    ...
    const errorResponse = ErrorResponseSchema.parse({
      error: "Failed to update archive views",
    });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from("page_views")
      .select("views")
      .eq("page", `/archive/${validatedParams.slug}`)
      .single();
    ...
    console.error("Error fetching archive views:", error);
